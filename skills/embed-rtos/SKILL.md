...
- OTA / update lifecycle → `embed-ota`
- Boot / partitioning → `embed-architect`
- Power management at system level → `embed-power`
...
- **Recommended next steps** — Return task table and sync primitives to the orchestrator; `pr-code-reviewer` reviews before proceeding. If task timing changes affect the power budget, coordinate with `embed-power`.
