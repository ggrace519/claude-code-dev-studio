...
- Feature implementation → `mobile-platform`
...
- **Recommended next steps** — After submission, monitor for review feedback. If the review is rejected for permissions, coordinate with `mobile-platform`. If rejected for billing or IAP, coordinate with `mobile-iap`. If rejected for privacy declarations, invoke `common-privacy`.
