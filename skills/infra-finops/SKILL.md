...
- SLO trade-offs (reliability vs cost) → `infra-sre` (joint)
- K8s workload sizing → `infra-k8s` (joint)
- Observability cardinality cost → `infra-observability` (joint)
...
- **Recommended next steps** — Return cost analysis to the orchestrator; `pr-code-reviewer` reviews code changes before merging. If reliability vs cost trade-offs need resolution, collaborate with `infra-sre`. If K8s workload sizing is the lever, collaborate with `infra-k8s`.
