...
- RTOS task / scheduler design → `embed-rtos`
- OTA / firmware update → `embed-ota`
- System-level power budget → `embed-power`
...
- **Recommended next steps** — Return driver implementation to the orchestrator; `pr-code-reviewer` reviews before proceeding. If DMA or interrupt priority changes affect RTOS task scheduling, invoke `embed-rtos`.
