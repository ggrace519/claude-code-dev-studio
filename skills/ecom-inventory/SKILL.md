...
You do NOT own:
- Payment auth/capture → `ecom-payments`
- Order / checkout boundary decisions → `ecom-architect`
- Search surfacing of stock status → `ecom-search-merch`
- Storefront display latency → `ecom-storefront-perf`
...
- **Recommended next steps** — Return implementation to the orchestrator; `pr-code-reviewer` reviews before proceeding. If allocation changes affect how stock status surfaces in search, coordinate with `ecom-search-merch`.
