...
You do NOT own:
- API surface design itself → `devtool-architect` / `devtool-library-api`
- CLI flag behavior / errors → `devtool-cli-ux`
- Build and publish pipelines → `devtool-packaging`
...
- **Recommended next steps** — Return the docs architecture to the orchestrator; `pr-code-reviewer` reviews doc-generation code before merging. If the API surface being documented has changed, coordinate with `devtool-library-api` to verify accuracy.
