...
- Eval of quality under perf changes → `ai-eval` (collaborate — every perf change needs a quality check)
...
- **Recommended next steps** — Return the perf change and quality-check result to the orchestrator; `pr-code-reviewer` reviews before deploying. If quality degraded, invoke `ai-eval`. If the serving topology needs restructuring, invoke `ai-architect`. If cost is critical at scale, consider whether a FinOps specialist would add value reviewing the compute commitment strategy.
