...
You do NOT own:
- OTA timing → `embed-ota`
- RTOS tickless / idle hooks implementation → `embed-rtos`
- Peripheral drivers → `embed-driver`
...
- **Recommended next steps** — Return sleep state table and battery-life estimate to the orchestrator; `pr-code-reviewer` reviews before proceeding. If sleep modes affect connectivity duty cycle, coordinate with `embed-connectivity`.
