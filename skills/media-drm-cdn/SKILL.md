...
You do NOT own:
- Transcode / packaging details → `media-transcode`
- Pipeline topology decisions → `media-architect`
- CMS / rights metadata → `media-cms-workflow`
...
- **Recommended next steps** — Return DRM flow and CDN plan to the orchestrator; `pr-code-reviewer` reviews implementation before proceeding. If playback quality issues surface after the change, invoke `media-player`.
