...
- Sync engine perf → `mobile-offline-sync` (collaborate)
...
- **Recommended next steps** — Return the fix to the orchestrator; `pr-code-reviewer` reviews before proceeding. If the regression guard subsequently triggers a crash spike, invoke `mobile-crash`.
