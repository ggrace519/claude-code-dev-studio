...
You do NOT own:
- Permission choice / model → `ext-permissions`
- Security / CSP for injected UI → `ext-security`
- Overall extension topology → `ext-architect`
- Generalist UI critique → `ux-design`
...
- **Recommended next steps** — Return UX spec to the orchestrator; `pr-code-reviewer` reviews implementation before proceeding. If in-page UI has accessibility issues, invoke `common-a11y`.
