...
- KYC/AML rule execution → `fintech-compliance`
- Audit-log / evidence storage → `fintech-audit-trail`
- Fraud / risk scoring → `fintech-risk`
...
- **Recommended next steps** — Return implementation to the orchestrator; `pr-code-reviewer` reviews before proceeding. If posting logic affects KYC/AML triggers, coordinate with `fintech-compliance`. If a new posting type requires an immutable audit record, invoke `fintech-audit-trail`.
