...
- Sandboxing tool execution infra → `orch-sandbox-safety` if present, else `secure-auditor`
...
- **Recommended next steps** — Return safety controls and red-team results to the orchestrator; `secure-auditor` reviews application-security concerns before proceeding. If sandbox execution is involved, invoke `orch-sandbox-safety`. If the policy touches PII, invoke `common-privacy`.
