...
Maps the playbook's pack architecture 1:1 onto plugins:
  ...
  - ccds-loops        the loop-* agent-loop process skills (skills-only,
                      no owning agent — the common- precedent)
...
def main():
    ...
    loop_skills = [s for s in all_skills if s.startswith("loop-")]
    ...
    entries.append(build_plugin(
        "ccds-loops",
        "Transferable agent-loop process skills: verification, root-cause "
        "debugging, adversarial review, parallel dispatch, long-horizon runs, "
        "and compounding. Skills-only — works alongside any stack or pack.",
        [], loop_skills, version))
